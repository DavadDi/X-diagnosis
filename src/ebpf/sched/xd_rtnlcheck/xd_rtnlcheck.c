...
int main(int argc, char **argv)
{
	int ret;
	void *event_pb = NULL;
	struct xd_rtnlcheck_bpf_c *skel;
	...
	skel = xd_rtnlcheck_bpf_c__open_and_load();
	if (!skel) {
	...
	/* Attach tracepoint */
	ret = xd_rtnlcheck_bpf_c__attach(skel);
	if (ret) {
...
cleanup:
	xd_rtnlcheck_bpf_c__destroy(skel);
	return ret;
}
